...
func TestJSON(t *testing.T) {
	...
	assert.Equal(t, "4kbr", config.GetString("app.author"))
	assert.Equal(t, "localhost", config.GetString("database.host"))
	assert.Equal(t, 8889, config.GetInt("database.port"))
...
func TestENVFile(t *testing.T) {
	...
	assert.Equal(t, "4kbr", config.GetString("APP_AUTHOR"))
	assert.Equal(t, "localhost", config.GetString("DATABASE_HOST"))
	assert.Equal(t, 8889, config.GetInt("DATABASE_PORT"))
...
func TestENV(t *testing.T) {
	...
	assert.Equal(t, "Hello", config.GetString("FROM_ENV")) //export FROM_ENV=Hello
}
